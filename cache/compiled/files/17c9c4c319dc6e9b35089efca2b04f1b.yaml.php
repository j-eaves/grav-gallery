...
return [
    ...
    'filename' => '/Users/catherder/My_WebApps/grav-admin_quark_theme/user/plugins/admin/languages/zh-cn.yaml',
    'modified' => 1549925968,
    'data' => [
        'PLUGIN_ADMIN' => [
            ...
            'JAVASCRIPT_PIPELINE_INCLUDE_EXTERNALS_HELP' => '外部的URLs有时有相对文件引用，不应管道化',
            ...
            'TWIG_FIRST' => '首先处理Twig',
            'TWIG_FIRST_HELP' => '如果你启用Twig页面处理，那么你能配置Twig处理先于或后于Markdown',
            'SESSION_SECURE' => '安全',
            'SESSION_SECURE_HELP' => '如果为 true，则指示此 cookie 的通信必须在加密传输。警告︰ 启用此功能只有在完全在 HTTPS 运行的网站上',
            'SESSION_HTTPONLY' => '仅限 HTTPS',
            'SESSION_HTTPONLY_HELP' => '如果为 true，则指示应仅通过 HTTP，使用 cookie 和不允许 JavaScript 修改',
            'REVERSE_PROXY' => '反向代理',
            'REVERSE_PROXY_HELP' => '如果你在反向代理之后，并且你的URL包含不正确的端口，启用此',
            ...
            'RESOURCE_FILTER' => '筛选器...',
            'FORCE_SSL' => '强制使用 SSL',
            'FORCE_SSL_HELP' => '全局强制 SSL，如果启用时网站达到通过 HTTP，Grav发送一个重定向到 HTTPS 页',
            'NEWS_FEED' => '官方博客',
            'EXTERNAL_URL' => '外部 URL',
            'SESSION_PATH' => '会话路径',
...
在您账户启用**两步验证**下。建议您使用**两步验证** 应用程序，输入当前**6位代码** 以便完成登录。',
            ...
            'TOOLS_DIRECT_INSTALL_UPLOAD_DESC' => '您可以通过此方法轻松安装有效的Grav  <strong>主题</ strong>，<strong>插件</ strong>，甚至是<strong> Grav </ strong>更新Zip包。这个软件包不需要通过GPM注册，并允许您轻松回滚到之前的版本或安装进行测试。',
